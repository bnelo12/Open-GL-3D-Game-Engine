...
void main() {
    ...
    if(hasEmissionMap) {
        emission = texture(emission_map, fragUVCoord)*vec4(emissionMapColour, 1);
    }
    
    //Ambient
    vec3 ambientCoefficient = light.ambient;
    ...
    finalColor =  vec4(diffuseCoefficient+specularCoefficient,1) + emission;
}
